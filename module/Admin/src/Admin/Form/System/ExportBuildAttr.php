...
class ExportBuildAttr extends Form
{
    ...
    protected $_sep_line = "\n";
    protected $_sep_cell = "\t";
    protected $_head = array('translate_code', 'exp');
    ...
    public function getDataForCsv() 
    {
        ...
        foreach ($libObjectRowset->getItems() AS $objectRow) {
            if ($objectRow->type == 'build') {
                $data[] = array(
                    'title' => $objectRow->code,
                );
                foreach ($objectRow->getLevelRowset()->getItems() AS $levelRow) {
                    ...
                    $data[] = array_merge(array($levelRow->level, $levelRow->exp), $attrData);
                }
            }
        }
        ...
        for ($i = 1; $i < count($csvData); $i++) {
            ...
            if(count($csvData[$i]) <= $count){
                ...
                if (!preg_match('/^(\d*)$/', $value, $match)) {
                    ...
                    if($value and $objectRow and isset($csvData[$i][1])){
                        $exp = intval($csvData[$i][1]);
                        if (!$levelRow = $objectRow->getLevelRowset()->getBy('level', $value))
                            throw new \Exception('Level not found['.$value.']');
                        else{
                            $levelRow->exp = $exp;
                            $levelRow->save();
                        }
