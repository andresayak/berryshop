...
class ObjectForm extends Form
{
    ...
    public function __construct($fountainRow, $objectRowset, $objectRow = false)
    {
        ...
        foreach($this->getObjectRowset()->getItems() AS $row){
            if($row->isCountType() 
                and (!$fountainRow->getObjectRowset()->getBy('object_code', $row->code)
